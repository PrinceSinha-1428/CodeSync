...
function InterviewScheduleUI() {
  ...
  const [formData, setFormData] = useState({
    ...
    time: "09:00",
    candidateId: "",
    interviewersId: user?.id ? [user.id] : [],
  });

  const scheduleMeeting = async () => {
    if (!client || !user) return;
    if (!formData.candidateId || formData.interviewersId.length === 0) {
  ...
  const addInterviewer = (interviewerId: string) => {
    if (!formData.interviewersId.includes(interviewerId)) {
      setFormData((prev) => ({
        ...prev,
        interviewerIds: [...prev.interviewersId, interviewerId],
      }));
    }
  };
  ...
  const removeInterviewer = (interviewerId: string) => {
    ...
    setFormData((prev) => ({
      ...prev,
      interviewerIds: prev.interviewersId.filter((id) => id !== interviewerId),
    }));
  };

  const selectedInterviewers = interviewers.filter((i) =>
    formData.interviewersId.includes(i.clerkId)
  );

  const availableInterviewers = interviewers.filter(
    (i) => !formData.interviewersId.includes(i.clerkId)
  );
